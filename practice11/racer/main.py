...
# bronze drops often, gold is rare
COIN_TYPES = [
    {'color': BRONZE, 'value': 1, 'weight': 60, 'radius': 10},
    {'color': SILVER, 'value': 2, 'weight': 30, 'radius': 12},
    {'color': YELLOW, 'value': 5, 'weight': 10, 'radius': 14},
]

SPEED_UP_PER_COINS = 10
...
# roll weighted random for coin type
def pick_coin_type():
...
def run():
    ...
    while True:
        ...
        if coins_collected // SPEED_UP_PER_COINS > last_speedup_threshold:
            last_speedup_threshold = coins_collected // SPEED_UP_PER_COINS
            enemy_speed += 1
        ...
        show_text(f"Speed: {enemy_speed}", 10, 40, WHITE, small_font)
